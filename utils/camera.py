...
from utils.helper1 import display_last_photo

# 'gui.py' dosyasının bulunduğu dizini al
current_dir = os.path.dirname(os.path.abspath(__file__))

# 'data' klasörüne gitmek için, 'utils' dışındaki ana proje klasöründen erişim sağla
data_folder = os.path.join(current_dir, '..', 'data')  # '..' bir üst klasöre çıkmayı sağlar
...
class CameraApp:
    ...
    def __init__(self):
        ...
        self.last_created_folder = load_last_created_folder()  # last_created_folder sınıf içinde yükleniyor
    ...
    def take_photo(self):
        # Burada self.last_created_folder kullanılıyor
        if self.last_created_folder is not None:
            photo_path = os.path.join(self.last_created_folder, "photo.jpg")  # Kaydedilecek fotoğrafın yolu
            _, frame = self.cap.read()
            frame = cv2.resize(frame, (500, 500))
            self.draw_center_frame(frame)
            ...
            display_last_photo(photo_path)  # Helper fonksiyonunu çağırarak fotoğrafı göster
        else:
            messagebox.showerror("Hata", "Last created folder bulunamadı!")
